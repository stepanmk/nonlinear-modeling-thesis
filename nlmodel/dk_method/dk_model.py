...
class LinearDKModel:
        ...
        for ch in range(in_samples.shape[1]):  # iterate through audio channels
            for sample in range(in_samples.shape[0]):  # iterate through channel samples
                ...
                if self.x.size == 0:
                    ...
                    self.x[:, ch : ch + 1] = (
                        self.A @ self.x[:, ch : ch + 1] + self.B @ self.u
                    )
    ...
    def create_state_space(self):
        ...
        if self.Nx.size == 0:
            ...
            So = torch.cat(
                [
                    So,
                    torch.cat(
                        [self.Nu, torch.zeros([n_voltage_sources, n_voltage_sources])],
                        dim=1,
                    ),
                ],
                dim=0,
        ...
        # if there are capacitors
            ...
            So = torch.cat(
                [
                    self.Nr.T @ self.Gr @ self.Nr + self.Nx.T @ self.Gx @ self.Nx,
                    self.Nu.T,
                ],
                dim=1,
            )
        ...
        Nvp = torch.cat(
            [self.Nv, torch.zeros([self.Nv.shape[0], n_voltage_sources])], dim=1
        )
        Nxp = torch.cat(
            [self.Nx, torch.zeros([self.Nx.shape[0], n_voltage_sources])], dim=1
        )
        Nup = torch.cat(
            [
                torch.zeros([n_real_nodes, n_voltage_sources]),
                torch.eye(n_voltage_sources),
            ],
            dim=0,
        )
        Nop = torch.cat(
            [self.No, torch.zeros([self.No.shape[0], n_voltage_sources])], dim=1
        )
        ...
        self.A = (
            ...
            * self.Z
            @ self.Gx
            @ self.Ux
            @ torch.linalg.inv(self.Rv + self.Q)
            @ self.Ux.T
        )
        self.B = (
            self.Bo
    ...
    def build_model(self, device=None):
        ...
        self.Nr = torch.zeros(
            [components_count["n_res"], components_count["n_nodes"]]
        )  # resistors
        self.Gr = torch.zeros([components_count["n_res"], components_count["n_res"]])

        if self.vres:
            self.Nv = torch.zeros(
                [components_count["n_vres"], components_count["n_nodes"]]
            )  # variable resistors
            self.Rv = torch.zeros(
                [components_count["n_vres"], components_count["n_vres"]]
            )
        else:
            self.Nv = []

        self.Nx = torch.zeros(
            [components_count["n_caps"], components_count["n_nodes"]]
        )  # capacitors
        ...
        self.Nu = torch.zeros(
            [components_count["n_inputs"], components_count["n_nodes"]]
        )  # inputs
        self.No = torch.zeros(
            [components_count["n_outputs"], components_count["n_nodes"]]
        )  # outputs
