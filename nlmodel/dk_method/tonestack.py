...
class JVMToneStack(LinearDKModel):
    def __init__(self, sr=44100, alpha_t=0.5, alpha_m=0.5, alpha_b=0.5, alpha_v=1.0):
        super().__init__()
        self.vres = True
        self.T = 1 / sr
...
class JVMToneStackSimple(LinearDKModel):
    def __init__(self, sr=44100, alpha_t=0.5, alpha_m=0.5, alpha_b=0.5):
        super().__init__()
        self.vres = True
        self.T = 1 / sr
        ...
        self.recompute_alphas(alpha_t, alpha_m, alpha_b)
        ...
        self.components = [
            Resistor("R_in", [1, 2], 4500),
            Resistor("R1", [2, 3], 33000),
            # TREBLE #
            VariableResistor("VR1_1", [4, 5], self.RT * self.alpha_t1),
            VariableResistor("VR1_2", [5, 6], self.RT * self.alpha_t2),
            # MID #
            VariableResistor("VR2_1", [7, 8], self.RM * self.alpha_m1),
            VariableResistor("VR2_2", [8, 0], self.RM * self.alpha_m2),
            # parallel constant resistors to avoid floating nodes
            Resistor("R2_1", [7, 8], self.RM * 2),
            Resistor("R2_2", [8, 0], self.RM * 2),
            # BASS #
            VariableResistor("VR3", [6, 7], self.RB * self.alpha_b),
            # VOLUME #
            Resistor("R5", [5, 9], 39000),
            Resistor("R6", [9, 0], 1000000),
            # capacitors
            Capacitor("C1", [2, 4], 470e-12),
            Capacitor("C2", [3, 6], 22e-9),
            Capacitor("C3", [3, 8], 22e-9),
            # input & output ports
            InputPort("In", [1, 0], 0),
            OutputPort("Out", [9, 0]),
        ]
    ...
    def recompute_alphas(self, t, m, b):
        self.alpha_t1 = 1 - t
        self.alpha_t2 = t
        self.alpha_m1 = (2 * (1 - m)) / (2 - (1 - m))
        self.alpha_m2 = (2 * m) / (2 - m)
