...
from ..dk_method.tonestack import JVMToneStack, JVMToneStackSimple
from .pot import CondBlock
...
class ToneStack(nn.Module):
    def __init__(
        ...
        self.alpha_rt = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
        self.alpha_rm = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
        self.alpha_rb = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
        ...
        self.alpha_c3 = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )

        # conditioning block
        self.cond_block = CondBlock(
    ...
    def get_component_values(self):
        alpha_rb = 0.9 + torch.sigmoid(self.alpha_rb) * 0.2
        alpha_rm = 0.9 + torch.sigmoid(self.alpha_rm) * 0.2
        alpha_rt = 0.9 + torch.sigmoid(self.alpha_rt) * 0.2
        alpha_r1 = 0.99 + torch.sigmoid(self.alpha_r1) * 0.02
        alpha_r2 = 0.99 + torch.sigmoid(self.alpha_r2) * 0.02

        alpha_c1 = 0.9 + torch.sigmoid(self.alpha_c1) * 0.2
        alpha_c2 = 0.9 + torch.sigmoid(self.alpha_c2) * 0.2
        alpha_c3 = 0.9 + torch.sigmoid(self.alpha_c3) * 0.2
        ...
        c_vals = {
            ...
            "res": {"R1": R1, "R2": R2, "RB": RB, "RM": RM, "RT": RT},
        }
        return c_vals
    ...
    def state_space_filt(self, u, processed_cond):
        ...
        for i in range(u.shape[1]):
            filt_out[:, i : i + 1, :] = (
                self.D @ self.recurse_state + self.E @ u[:, i : i + 1, :]
            )
            self.recurse_state = (
                self.A @ self.recurse_state + self.B @ u[:, i : i + 1, :]
            )
        return filt_out

    def update_components(self, processed_cond):
        RB = (0.9 + torch.sigmoid(self.alpha_rb) * 0.2) * self.RB
        RM = (0.9 + torch.sigmoid(self.alpha_rm) * 0.2) * self.RM
        RT = (0.9 + torch.sigmoid(self.alpha_rt) * 0.2) * self.RT
    ...
    def update_resistors(self, RM):
        ...
        R2 = (0.99 + torch.sigmoid(self.alpha_r2) * 0.02) * self.R2
        Gr[0, 0] = 1 / R1
    ...
    def update_caps(self):
        ...
        C3 = (0.9 + torch.sigmoid(self.alpha_c3) * 0.2) * self.C3
    ...
    def reset_states(self):
        ...
        self.recurse_state = torch.zeros((self.batch_size, 3, 1), device=self.device)
...
class ToneStackSimple(nn.Module):
    ...
    def __init__(
        ...
        self.ts = JVMToneStackSimple(sr=self.sr)
        # matrices for state space computation
        self.A, self.B, self.D, self.E = None, None, None, None
        # resistors and caps
        self.Nr, self.Gr, self.Nx, self.Gx = None, None, None, None
        ...
        self.alpha_r_in = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
        self.alpha_r1 = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
        self.alpha_r5 = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
        self.alpha_r6 = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
    ...
    def update_resistors(self, RM):
        ...
        Gr = torch.zeros_like(self.Gr, device=self.device)
        # 1% tol
        R_in = (0.9 + torch.sigmoid(self.alpha_r_in) * 0.2) * self.R_in
        R1 = (0.99 + torch.sigmoid(self.alpha_r1) * 0.02) * self.R1
        R5 = (0.99 + torch.sigmoid(self.alpha_r5) * 0.02) * self.R5
        R6 = (0.9 + torch.sigmoid(self.alpha_r5) * 0.2) * self.R6
        Gr[0, 0] = 1 / R_in
        Gr[1, 1] = 1 / R1
        # mid parallel resistors
        Gr[2, 2] = 1 / (RM * 2)
