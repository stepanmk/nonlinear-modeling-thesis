...
from ..dk_method.ds1_tone import DS1Tone
...
class DS1(nn.Module):
    ...
    def __init__(
        ...
        self.alpha_r3 = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
        self.alpha_r4 = nn.Parameter(
            torch.tensor([0.0], device=self.device), requires_grad=True
        )
    ...
    def update_components(self, processed_cond):
        ...
        self.update_pots(processed_cond, RT)
        self.update_resistors()
        self.update_caps()

    def update_pots(self, processed_cond, RT):
        ...
        # TREBLE
        Rv[:, 0, 0] = processed_cond[0] * RT
        Rv[:, 1, 1] = (1 - processed_cond[0]) * RT
    ...
    def update_resistors(self):
        ...
        R1 = (0.9 + torch.sigmoid(self.alpha_r1) * 0.2) * self.R1
        R2 = (0.9 + torch.sigmoid(self.alpha_r2) * 0.2) * self.R2
        R3 = (0.9 + torch.sigmoid(self.alpha_r3) * 0.2) * self.R3
        R4 = (0.9 + torch.sigmoid(self.alpha_r4) * 0.2) * self.R4
        Gr[0, 0] = 1 / R1
        Gr[1, 1] = 1 / R2
        Gr[2, 2] = 1 / R3
        Gr[3, 3] = 1 / self.R4
